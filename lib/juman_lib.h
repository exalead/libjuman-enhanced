...
JEXPORT int      take_data(JCONTEXT* ctx, int pos, char **pbuf);
JEXPORT char *	_take_data(JCONTEXT* ctx, char *s, MRPH *mrph);
...
/*
  ...
  NACSIS 吉岡
*/   
JEXPORT void	print_path_mrph(JCONTEXT* ctx,
                        FILE* output, int path_num , int para_flag);
...
JEXPORT void	_print_all_path(JCONTEXT* ctx, FILE* output, int path_num,
                                int pathes);
JEXPORT char	**print_homograph_path(JCONTEXT* ctx, FILE* output);
JEXPORT int	_print_homograph_path(JCONTEXT* ctx,
                              FILE* output, const int level, int pbuf_start, int new_p);

JEXPORT int	pos_match_process(JCONTEXT* ctx, int pos, int p_start);
JEXPORT int	pos_right_process(JCONTEXT* ctx, int position);
JEXPORT int	check_connect(JCONTEXT* ctx, int pos_start, int m_num);
