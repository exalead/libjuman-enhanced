...
#ifdef _WIN32
/* [MSDN] "However, calling this function simultaneously from multiple threads
...
*
****************************************************/
pat_node *pat_search(char* buffer, DIC_FILE_ENTRY *f,
                     char *key, pat_node *x_ptr, char *rslt)
{
...
/*  rslt[0] = '\0';*/
  ...
  do {
	...
	tmp_x_ptr = tmp_x_ptr->left;
      ...
      strtok_r(buffer, "\t", &strtoktmp); /* 最初の '\t' を '\0' にする．*/
...
*   検索終了位置(ポインタ)
****************************************************/
pat_node *pat_search_exact(DIC_FILE_ENTRY *f, char *key, pat_node *x_ptr, char *rslt)
{
  pat_node *ptr,*tmp_ptr,*top_ptr = x_ptr,*tmp_x_ptr = x_ptr;
  pat_index_list *tmp_l_ptr;
  int in_hash;
...
*   無し!
****************************************************/
void pat_insert(DIC_FILE_ENTRY *f,
                char *line, long index, pat_node *x_ptr, char *kugiri)
{
  ...
  OL(line:)OS(line);
  strcpy(key,line);
  strtok_r(key,kugiri, &strtoktmp);  /* 最初の区切り文字を '\0' にする．*/
...
*   -1 : 失敗
****************************************************/
const char *get_line(DIC_FILE_ENTRY *f, long pos){
  int i = 0, j = 0, ch;
...
/*
    ...
    top_ptr->left->str = (char*)my_alloc(strlen(word)+1);
    strcpy(top_ptr->left->str,word);
*/
    top_ptr->left->str = (char*)my_alloc(strlen(pftmp)+1);
...
/*
    ...
    strcpy(top_ptr->left->str,word);
*/
      top_ptr->right->str = (char*)my_alloc(strlen(pftmp)+1);
