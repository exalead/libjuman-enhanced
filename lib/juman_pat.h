#ifndef JUMAN_PAT_H_DEF
#define JUMAN_PAT_H_DEF
...
typedef struct DIC_FILE_ENTRY_ {
  union {
    FILE *fp;
    char *mm;
    /* Opaque pointer */
...
extern DIC_FILE_ENTRY* dic_file[MAX_DIC_NUMBER]; /* 木のもとデータ(辞書ファイル) */
...
extern pat_node *pat_search(char* tmpbuff, DIC_FILE_ENTRY*,char*,pat_node*,char*); /* パトリシア木で検索 */
extern pat_node *pat_search4insert(char*,pat_node*); /* 挿入用検索 */
extern void pat_insert(DIC_FILE_ENTRY*,char*,long,pat_node*,char*); /* パトリシア木に挿入 */
extern int pat_bits(char*,int,int); 
         /* 文字列中の指定された位置のビットを返す */
         /* 960918  内部でstrlrn()をやるのは無駄であることが判明 */
extern void show_pat(pat_node*,FILE*,char*); /* パトリシア木データを出力 */
extern const char *get_line(DIC_FILE_ENTRY*,long); /* 指定された場所から'\n'まで読む */
...
extern void insert_dic_data(DIC_FILE_ENTRY*,pat_node*,char*); /* 辞書データを挿入*/
