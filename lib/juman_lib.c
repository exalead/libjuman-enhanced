...
static BOOL    katuyou_process(JCONTEXT* ctx,
                               int position, int *k, MRPH mrph, int *length);
...
int search_all(JCONTEXT* ctx, int position)
{
	...
	while (*pbuf != '\0') {
	    if (take_data(ctx, position, &pbuf) == FALSE) return FALSE;
	}
    }
    return TRUE;
}
...
------------------------------------------------------------------------------
*/
int take_data(JCONTEXT* ctx, int pos, char **pbuf)
{
    unsigned char    *s;
    int     i, k, f, num;
...
/*printf(".....%d/%d   %s %d\n", i, num, new_mrph->midasi, new_mrph->con_tbl);*/
	...
	if (Class[new_mrph->hinsi][new_mrph->bunrui].kt == 0) {
		...
		co = 0;
		while (katuyou_process(ctx, pos, &k2, *new_mrph, &length)) {
                    assert(co >= 0
...
/*printf("  TO %d\n", new_mrph->con_tbl);*/
		    ...
		    if (check_connect(ctx, pos, ctx->m_buffer_num) == FALSE)
                      return FALSE;
	...
	if ( Class[mrph.hinsi][mrph.bunrui].kt ) { /* 活用する */
		...
		k2 = 1;
		while (katuyou_process(ctx, pos, &k2, mrph, &length)) {
                    assert(ctx->m_buffer_num < ctx->mrph_buffer_max);
...
char *_take_data(JCONTEXT* ctx, char *s , MRPH *mrph)
{
    int		i, j, k = 0;
    char	c;
...
{
    if ((mrph_p->hinsi == JOpt.kakko_hinsi &&
	 (mrph_p->bunrui == JOpt.kakko_bunrui1 || 
	  mrph_p->bunrui == JOpt.kakko_bunrui2))||
	(mrph_p->hinsi == JOpt.kuuhaku_hinsi && 
	 mrph_p->bunrui == JOpt.kuuhaku_bunrui))
	return TRUE;
    else 
	return FALSE;
}
...
char *get_path_mrph(JCONTEXT* ctx, int path_num , int para_flag)
{
    ...
    if ((mrph_p = prepare_path_mrph(ctx, path_num, para_flag)) == NULL)
      return NULL;
    len = strlen(ctx->kigou)
      +strlen(ctx->midasi1)
      +strlen(ctx->yomi)
      +strlen(ctx->midasi2)
      +strlen(Class[mrph_p->hinsi][0].id)
      +mrph_p->hinsi/10+1;
...
int check_connect(JCONTEXT* ctx, int pos, int m_num)
{
    int chk_con_num;
    int i, j, pathes;
	...
	* new_mrph->weight * JOpt.cost_omomi.keitaiso;
    ...
    assert(ctx->p_buffer_num >= 0
           && ctx->p_buffer_num < ctx->process_buffer_max);
    ctx->p_buffer[ctx->p_buffer_num].path[0]
      = ctx->chk_connect[best_score_num].pre_p;
    pathes = 1;
...
int juman_sent(JCONTEXT* ctx)
{
    ...
    if (check_connect(ctx, length, ctx->m_buffer_num-1) == FALSE)
      return FALSE;
