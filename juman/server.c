...
/*
  ...
  NACSIS 吉岡
*/
static int do_juman(JCONTEXT* ctx)
{
    char *end_of_line;
    int ret;

    /* fgets() のチェックのための番人 */
    end_of_line = ctx->String + ctx->String_max - 1;
    *end_of_line = '\n';

    while (fgets(ctx->String, ctx->String_max,
                 client_ifp) != NULL) {
        if (*end_of_line != '\n') {
	    fputs("line too long\n", client_ofp);
	...
	if ((JOpt.Show_Opt_tag)[0])
		...
		fprintf(client_ofp, "%s", ctx->String);
		fflush(client_ofp);
		continue;
	    }
        ...
        if (strlen(ctx->String) != 0)
          {
            ...
            case Op_B:  
              print_best_path(ctx, client_ofp); fprintf(client_ofp, "EOS\n"); fflush(client_ofp); break;
            case Op_M: 
              print_all_mrph(ctx, client_ofp); fprintf(client_ofp, "EOS\n"); fflush(client_ofp); break;
            case Op_P: 
              print_all_path(ctx, client_ofp); fprintf(client_ofp, "EOS\n"); fflush(client_ofp); break;
            case Op_BB: 
            case Op_PP: 
              print_homograph_path(ctx, client_ofp); fprintf(client_ofp, "EOS\n"); fflush(client_ofp); break;
            default:   break;
            }
          }
#if 0
...
int option_proc_for_server(JCONTEXT* ctx, int argc, char **argv)
{
    int 	i;
	...
	if (argv[i][0] != '-') {
	    ...
	    else if ( argv[i][1] == 'i' ) strcpy(JOpt.Show_Opt_tag,
                                                 argv[i+1]), i++;
...
/*
  ...
  NACSIS 吉岡
*/
static int juman_run(ctx, argc, argv)
     int argc;
     char **argv;
     JCONTEXT *ctx;
{
    ...
    if (!option_proc_for_server(ctx, argc, argv)) {
 	skip_until_EOf(client_ifp);
    ...
    JCONTEXT* ctx;
{
    ...
    if (!strcasecmp(argv[0], "RUN"))
      return juman_run(ctx, argc, argv);
